...
static Result destroyAllChallenges(ChallengeRoomSystem *sys) {
	if (!sys) {
...
static Result destroyAllRooms(ChallengeRoomSystem *sys) {
	if (!sys) {
		return NULL_PARAMETER;
	}
...
static struct visitor_node* createVisitorNode(Visitor* visitor) {
...
static void addVisitorToList(ChallengeRoomSystem *sys, Visitor* visitor) {
	struct visitor_node *new_node = createVisitorNode(visitor);
	if (!sys->visitor_list) {
...
static void removeVisitorFromList(ChallengeRoomSystem *sys, Visitor* visitor) {
...
static Result destroyVisitorListNode(struct visitor_node *ptr) {
	while (ptr) {
...
static struct visitor_node* findVisitorNode(
...
static struct visitor_node* findVisitorNodeByName(
...
static Result initSysVariables(ChallengeRoomSystem** sys) {
...
static Result readChallengesFromFileAndParse(FILE* fd,
...
static Result linkChallengesToRooms(FILE* fd, ChallengeRoomSystem **sys,
		int num_of_challenges_in_room, int* challenge_number_in_room,
		int iterator) {
	...
	for (int x = 0; x < num_of_challenges_in_room; x++) {
		for (int y = 0; y < (*sys)->number_of_challenges; y++) {
			if (challenge_number_in_room[x] == (*sys)->all_challenges[y].id) {
				Result res2 = init_challenge_activity(
						&((*sys)->all_rooms[iterator].challenges[x]),
...
static Result createRoomsFromFile(FILE* fd, ChallengeRoomSystem **sys,
		int number_of_rooms) {
	...
	for (int i = 0; i < number_of_rooms; i++) {
		...
		free(room_name);
		res1 = linkChallengesToRooms(fd, sys, num_of_challenges_in_room,
				challenge_number_in_room, i);
		if (res1 != OK) {
			return res1;
		}
		free(challenge_number_in_room);
...
static Result readRoomsFromFileAndParse(FILE* fd, ChallengeRoomSystem **sys) {
	// scan the amount of room from file to system variable
	int number_of_rooms = 0;
	fscanf(fd, "%d", &number_of_rooms);
	(*sys)->number_of_rooms = number_of_rooms;
	// allocate memory for every all rooms in the system rooms array
	(*sys)->all_rooms = malloc(sizeof(ChallengeRoom) * number_of_rooms);
	if (!(*sys)->all_rooms) {
		free((*sys)->all_challenges);
		fclose(fd);
		return MEMORY_PROBLEM;
	}
	Result r = createRoomsFromFile(fd, sys, number_of_rooms);
	return r;
}

static Result readFileAndParse(FILE* fd, ChallengeRoomSystem **sys) {
	fscanf(fd, "%s", (*sys)->system_name); //scan system name
	Result r = readChallengesFromFileAndParse(fd, sys);
	if (r != OK) {
		return r;
	}
	r = readRoomsFromFileAndParse(fd, sys);
	if (r != OK) {
		return r;
	}
	return OK;
...
// checking errors
	...
	FILE* fd = fopen(init_file, "r");		//open file
	if (!fd) {
		return NULL_PARAMETER;
	}
	*sys = malloc(sizeof(**sys));
	...
	Result r = initSysVariables(sys);
	if (r != OK) {
		return r;
	}
	// static function the scan the HomeWork File
	readFileAndParse(fd, sys);
	fclose(fd);
	return OK;
}
...
Result destroy_system(ChallengeRoomSystem *sys, int destroy_time,
	...
	for (int i = 0; i < sys->number_of_challenges; i++) {
		...
		if (best_time_holder == 0 || time_holder < best_time_holder) {
			...
			*challenge_best_time = stringDuplicate(sys->all_challenges[i].name);
			...
			if (strcmp(*challenge_best_time, sys->all_challenges[i].name) < 0) {
				free(*challenge_best_time);
				*challenge_best_time = stringDuplicate(
	...
	destroyAllRooms(sys);
	destroyAllChallenges(sys);
	destroyVisitorListNode(sys->visitor_list);
...
Result visitor_arrive(ChallengeRoomSystem *sys, char *room_name,
	...
	struct visitor_node *v = findVisitorNode(sys->visitor_list, visitor_id);
	if (v != NULL) {
...
Result visitor_quit(ChallengeRoomSystem *sys, int visitor_id, int quit_time) {
	...
	struct visitor_node *visitor_node = findVisitorNode(sys->visitor_list,
			visitor_id);
	...
	removeVisitorFromList(sys, visitor_node->visitor_n);
...
Result system_room_of_visitor(ChallengeRoomSystem *sys, char *visitor_name,
	...
	struct visitor_node *my_node = findVisitorNodeByName(sys->visitor_list,
			visitor_name);
	if (!my_node) {
		return NOT_IN_ROOM;
	}
	...
	*challenge_name = NULL;
	...
	for (int i = 0; i < sys->number_of_challenges; i++) {
		...
		if (popularity_holder > most_popular_holder) {
			free(*challenge_name);
			*challenge_name = stringDuplicate((sys->all_challenges[i].name));
			...
			if (*challenge_name == NULL) {
				...
				*challenge_name = stringDuplicate(sys->all_challenges[i].name);
				...
				if (strcmp(sys->all_challenges[i].name, *challenge_name) < 0) {
					free(*challenge_name);
					*challenge_name = stringDuplicate(
							sys->all_challenges[i].name);
					if (*challenge_name == NULL) {
						return MEMORY_PROBLEM;
					}
